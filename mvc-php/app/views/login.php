...
<body class="bg-primary min-h-screen flex items-center justify-center">
    <div class="w-full max-w-md p-8 space-y-6 bg-white/5 rounded-lg shadow-md">
        <h1 class="text-2xl font-bold text-center text-primary-foreground">Connexion</h1>
        <?php if (isset($_GET['error'])): ?>
            <p class="text-red-400 text-center">Nom d'utilisateur ou mot de passe incorrect.</p>
        <?php endif; ?>
        <form method="POST" action="/login" class="space-y-6">
            ...
            <button type="submit" class="w-full py-2 px-4 bg-accent text-white rounded-md hover:bg-[#581010]">Se connecter</button>
        </form>
        <p class="text-sm text-center text-primary-foreground/70">
            Pas encore de compte ? <a href="/register" class="font-medium text-accent hover:underline">Inscrivez-vous</a>
        </p>
    </div>
</body>
</html>
